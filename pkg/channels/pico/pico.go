...
// secureCompare performs a constant-time comparison while hiding length.
// It executes a dummy comparison if lengths differ to prevent length timing leaks.
func secureCompare(given, actual string) bool {
	if len(given) != len(actual) {
		subtle.ConstantTimeCompare([]byte(actual), []byte(actual))
		return false
	}

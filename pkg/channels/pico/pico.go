...
// secureCompare performs a timing-safe comparison of two byte slices.
// It executes a dummy comparison when lengths differ to prevent length leakage.
func secureCompare(given, actual []byte) bool {
	if len(given) != len(actual) {
		// Dummy comparison to equalize timing based on the actual secret's length.

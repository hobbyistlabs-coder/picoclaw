...
func NewAgentInstance(
	...
	if rc := defaults.Routing; rc != nil && rc.Enabled && rc.LightModel != "" {
		...
		if len(resolved) > 0 {
			...
			logger.WarnCF("routing", "light_model not found in model_list — routing disabled", map[string]any{
				"agent":       agentID,
...
func initSessionStore(dir string) session.SessionStore {
	...
	if err != nil {
		logger.WarnCF("memory", "init store failed; using json sessions", map[string]any{"error": err.Error()})
	...
	if n, merr := memory.MigrateFromJSON(context.Background(), dir, store); merr != nil {
		...
		logger.WarnCF("memory", "migration failed; falling back to json sessions", map[string]any{"error": merr.Error()})
		store.Close()
		return session.NewSessionManager(dir)
	} else if n > 0 {
		logger.InfoCF("memory", "migrated session(s) to jsonl", map[string]any{"count": n})
	}

...
## 2025-02-18 - String Concatenation in Loops
**Learning:** Found an instance in `pkg/agent/async_batches.go` where string concatenation (`+=`) was used inside a loop to join `state.results` into a single string. This is a classic $O(N^2)$ memory allocation anti-pattern in Go, as a new string backing array must be allocated and copied on every iteration.
**Action:** Replaced the loop with `strings.Join(state.results, "\n\n")`. `strings.Join` calculates the required string length upfront and allocates the memory exactly once, reducing memory overhead and time complexity to $O(N)$.

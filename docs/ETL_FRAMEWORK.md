...
- **Pipeline Extractor:** A background pipeline (`pkg/etl/pipeline.go`) periodically extracting runtime telemetry (`runtime.MemStats` and `runtime.NumGoroutine()`) and dumping it as JSONL into `{workspacePath}/logs/etl/system_metrics.jsonl`.
